...
export default function UserTestForm() {
  ...
  function handleArrayChange(e: React.ChangeEvent<HTMLTextAreaElement>, campo: keyof typeof camposPadrao) {
    setForm({ ...form, [campo]: e.target.value.split('\n').map(l => l.trim()).filter(Boolean) });
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 500, margin: '0 auto', padding: 24, background: '#fafafa', borderRadius: 8 }}>
      ...
      <label>Versão do sistema:<br />
        <input name="versao" value={form.versao} onChange={handleChange} required />
      </label><br />
      <label>Cenário testado:<br />
        <input name="cenario" value={form.cenario} onChange={handleChange} required />
      </label><br />
      <label>Passos realizados:<br />
        <textarea name="passos" value={form.passos.join('\n')} onChange={e => handleArrayChange(e, 'passos')} rows={3} />
      </label><br />
      <label>Resultados observados:<br />
        <textarea name="resultados" value={form.resultados.join('\n')} onChange={e => handleArrayChange(e, 'resultados')} rows={2} />
      </label><br />
      <label>Checklist de validação:<br />
        <textarea name="checklist" value={form.checklist.join('\n')} onChange={e => handleArrayChange(e, 'checklist')} rows={2} />
      </label><br />
      <label>Feedback do usuário:<br />
